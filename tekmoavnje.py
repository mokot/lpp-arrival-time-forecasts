...
from homework04.lpputils import parsedate, tsdiff, tsadd
from generiraj.linear import LinearLearner
...
MESTNI_LOG_VIZMARJE_PREDICTION = 60
...
def calculate_mae(data, prediction):
    ...
    return np.mean(
        np.abs(calculate_duration(data[:, 6], data[:, 8]) - prediction))
...
def get_time_week(data, interval=3):
    ...
    for i, date in enumerate(data):
        time[i, (parsedate(date).weekday() * 24 + parsedate(date).hour) //
             interval] = 1
    return time.T
...
def get_time_day(data, interval=5):
    ...
    for i, date in enumerate(data):
        time[i, (parsedate(date).hour * 60 + parsedate(date).minute) //
             interval] = 1
    return time.T
...
def get_holiday(data):
    ...
    """
    return np.asanyarray(
        [0 if (HOLIDAYS.get(date) != None) else 1 for date in data])
    ...
    return np.asanyarray([
        [
            (
                ...
                0 if forecast[5] == "da" else 1, ) for forecast in weather
...
def get_vehicles_poly(data):
    ...
    return np.asanyarray(
        [vehicle_ids.index(vehicle_id) for vehicle_id in data])
...
def get_drivers_poly(data):
    ...
    return np.asanyarray(
        [driver_ids.index(int(driver_id)) for driver_id in data])
...
def get_route(data):
    ...
    for i, route_direction in enumerate(data):
        ...
        routes[i, (route_id if
                   (route_directions[route_id - 1] == route_direction[1]) else
                   (2 * route_id)) - 1] = 1
...
def calculate_duration(departure, arrival):
    ...
    @return: the duration
    """
    return np.asanyarray([(tsdiff(arrival, departure))
                          for departure, arrival in zip(departure, arrival)])
...
def get_model(model, train_data, test_data):
    ...
    NUM_OF_DRIVERS = np.max(
        np.concatenate((filter_train_data[:, 1], filter_test_data[:, 1]),
    ...
    # linear_regression = LinearLearner(lambda_=1.)
    # predict = linear_regression(Xsp.T, y)

    # # calculate the first predicted data
    # predicted_data = [
    #     predict(prediction)
    #     for prediction in np.transpose(get_matrix(filter_test_data))
    # ]

    # we want to predict the arrival time (duration)
    xgb_model = xgb.XGBRegressor(objective="reg:squarederror",
                                 random_state=42).fit(X.T, y)
    # calculate the second predicted data
    predicted_data = xgb_model.predict(get_matrix(filter_test_data).T)

    # predicted_data1 = np.copy(predicted_data)
    # # we want to predict the arrival time (duration)
    # linear_regression = LinearRegression()
    # model = linear_regression.fit(Xsp.T, y)
    # # print("Model accuracy: ", model.score(Xsp.T, y))
    # # calculate the second predicted data
    # predicted_data2 = linear_regression.predict(get_matrix(filter_test_data).T)

    # predicted_data = []
    # for index in range(len(predicted_data1)):
    #     # check if predicted_data2 is extremely larger than predicted_data1
    #     if np.abs(predicted_data2[index]) - np.abs(predicted_data1[index]) > K:
    #         predicted_data.append(predicted_data1[index])
    #     else:
    #         predicted_data.append(predicted_data2[index])
...
if __name__ == "__main__":
    ...
    train_data = filter_month(train_data, month=[1, 2, 3, 10,
                                                 11])  # filter by month
    ...
    models_train = np.asanyarray([
        ...
        for temp_data in train_data[:, (2, 3)]
    ])
    for model in models_train:
        if not model in predicted_dict.keys():
            predicted_dict[model] = list(
                get_model(model, train_data, test_data))

    models_test = np.asanyarray([
        temp_data[0] + ": " + temp_data[1]
        for temp_data in test_data[:, (2, 3)]
    ])
    predicted_data = []
    ...
    write_data("tekmovanje.txt", test_data, predicted_data)