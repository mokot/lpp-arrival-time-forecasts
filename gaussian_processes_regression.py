...
def get_model(model, train_data, test_data):
    ...
    predicted_data1 = linear_regression.predict(get_matrix(filter_test_data).T)
    ...
    predicted_data2 = gaussian_process_regression.predict(
        get_matrix(filter_test_data).T
    )

    predicted_data = []
    for index in range(len(predicted_data1)):
        # check if predicted_data2 is extremely larger than predicted_data1
        if np.abs(np.abs(predicted_data2[index]) - np.abs(predicted_data1[index])) > K:
...
if __name__ == "__main__":
    ...
    for model in models_train:
        if not model in predicted_dict.keys():
            predicted_dict[model] = list(get_model(model, train_data, test_data))
    ...
    write_data("gaussian_process_regression.txt", test_data, predicted_data)
